...
public class GetWeatherTenDayTest extends AbstractTest{
    ...
    void get_shouldReturn500() throws IOException {
        ...
        //given
        logger.debug("Формирование мока для GET /forecasts/v1/daily/15day/290396");
        stubFor(get(urlPathEqualTo("/forecasts/v1/daily/15day/290396"))
                .willReturn(aResponse()
                        .withStatus(500).withBody("ERROR")));
        CloseableHttpClient httpClient = HttpClients.createDefault();
        HttpGet request = new HttpGet(getBaseUrl()+"/forecasts/v1/daily/15day/290396");
        logger.debug("http клиент создан");
        //when
        HttpResponse response = httpClient.execute(request);
        //then
        verify(getRequestedFor(urlPathEqualTo("/forecasts/v1/daily/15day/290396")));
